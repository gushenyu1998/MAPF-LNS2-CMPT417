...
def agent_random_walk(path_list_tuple, collision_list, map):
    ...
    if len(random_agent['path']) < t:
        t = len(random_agent['path'])
    ...
    while True:
        ...
        temp_prev = random_agent['path'][walk_time]
        temp = None
        if move == 'left':
            ...
            if walk_time + 1 > (map['map']['dimensions'][0] * map['map']['dimensions'][1]):
                return None
            random_agent['path'].append(temp)
            walk_time += 1
            continue
        ...
        for agent_other in path_list_tuple:
            if agent_other['agent'] == agent:
                continue
            loc = get_location(agent_other['path'], walk_time + 1)
            loc_prev = get_location(agent_other['path'], walk_time)
            if random_walk_detect_collision(temp, loc, temp_prev, loc_prev) == "V collision":
                random_agent['path'].append(temp)
                temp_vertice = {'agent': random_agent['agent'], 'loc': [temp]}
                # print("random walk detect vector collision, collision place: ", temp_vertice)
...
def check_random_movement_constraint(agent, temp, temp_prev, map):
    ...
    #             if temp['x'] == constraint['loc'][0]['x'] and temp['y'] == constraint['loc'][0]['y'] and temp['t'] == \
    #                     constraint['loc'][0]['t']:
    #                 return False
    #         elif len(constraint['loc']) == 2:
    #             if temp_prev['x'] == constraint['loc'][0]['x'] and\
    #                     temp_prev['y'] == constraint['loc'][0]['y'] and \
    #                     temp_prev['t'] == constraint['loc'][0]['t'] and \
    #                     temp['x'] == constraint['loc'][1]['x'] and \
    #                     temp['y'] == constraint['loc'][1]['y'] and \
    #                     temp['t'] == constraint['loc'][1]['t']:
    #                 return False
    return True
...
def detect_collision(path1, path2):
    ...
    for i in range(max_size):
        if get_location(path_1, i) == get_location(path_2, i):
            ...
            collision = {'a1': path1['agent'],
                         'a2': path2['agent'],
                         'loc': [path_1[min(i, len(path_1) - 1)], path_1[min(i + 1, len(path_1) - 1)]],
                         'timestep': i + 1
                         }
            collision_list.append(collision)
...
def main():
    global result
    for file in sorted(glob.glob((r'./LNS_instance/test_*.yaml'))):
        print(file)
        with open(file) as map_file:
            ...
            for j in range(len(path_agent_combination)):
                collisons = detect_collision(path_agent_combination[j][0], path_agent_combination[j][1])
                if len(collisons) != 0:
                    for colli in collisons:
                        Collision_list.append(colli)

            if len(Collision_list) == 0:
                print('There is no collision')
                time_end = time.time()
                result['time'].append(time_end - time_start)
                max_cost = 0
                for path in path_agent_tuples:
                    if len(path['path']) > max_cost:
                        max_cost = len(path['path'])
                result['max_cost'].append(max_cost)
                continue
            ...
            result_single_test = 0
            # Contribute constraint list
            max_cost = 0
            while len(As_list) != 0:
                find_result = agent_random_walk(path_agent_tuples, Collision_list, map)
                if find_result is not None:
                    result_single_test += 1
                    if len(find_result['path']) > max_cost:
                        max_cost = len(find_result['path'])
                else:
                    continue
            time_end = time.time()
            result['time'].append(time_end - time_start)
